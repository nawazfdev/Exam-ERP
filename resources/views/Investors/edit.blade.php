...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">{{ __('Edit Investor') }}</h1>
            </div>
            <div class="col-sm-6 text-right">
                <a href="{{ route('Investors.index') }}" class="btn btn-primary">{{ __('Back to Investors') }}</a>
            </div>
        </div>
    </div>
</div>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <form action="{{ route('Investors.update', $investor->id) }}" method="POST"
                        ...
                        <div class="card-body">
                            <!-- Product Title Field -->
                            <label for="title">{{ __('Investor Title') }}</label>
                            <div class="input-group mb-3">
                                <input type="text" name="title" id="title"
                                    class="form-control @error('title') is-invalid @enderror"
                                    placeholder="{{ __('Enter Investor Title') }}"
                                    value="{{ old('title', $investor->title) }}" required>
                                @error('title')
                            ...
                            @if($investor->feature_image)
                                <img src="{{ asset('storage/' . $investor->feature_image) }}" alt="Feature Image"
                                    class="img-thumbnail" width="150">
                            @endif
<br>
                            <!-- Description Field with Summernote -->
                            <label for="description">{{ __('Investor Description') }}</label>
                            <div class="input-group mb-3">
                                <textarea id="summernote" name="description"
                                    class="form-control @error('description') is-invalid @enderror">
                                    {{ old('description', $investor->description) }}
                                </textarea>
                                @error('description')
                                    <span class="error invalid-feedback">{{ $message }}</span>
                                @enderror
