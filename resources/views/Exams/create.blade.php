...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('Create Exams') }}</h1>
                </div>
                                                ...
                                                <div class="form-group">
                                                    @if ($field === 'negative_marks')
                                                        ...
                                                        <select name="{{ $field }}" id="{{ $field }}" class="form-control">
                                                            ...
                                                            <option value="10%">10% Apply</option>
                                                        </select>
                                                    ...
                                                    @elseif ($field === 'submit_before_end' || $field === 'display_countdown')
                                                        ...
                                                        <div>
                                                            <input type="radio" name="{{ $field }}" value="1"> Yes
                                                            <input type="radio" name="{{ $field }}" value="0"> No
                                                        </div>

                                                    @elseif ($field === 'availability')
                                                        <label>{{ __($label) }}</label>
                                                        <div>
                                                            <input type="radio" name="{{ $field }}" value="always"> Always
                                                            <input type="radio" name="{{ $field }}" value="specific_time"> Available on
                                                            Specific Time
                                                        </div>
                                                    ...
                                                    @elseif ($field === 'candidate_group_id')
                                                        ...
                                                        <!-- <select name="{{ $field }}" id="{{ $field }}" class="form-control">
                                                            ...
                                                            @foreach ($candidateGroups as $group)
                                                                <option value="{{ $group->id }}" {{ old($field) == $group->id ? 'selected' : '' }}>
                                                                    {{ $group->name }}
                                                                </option>
                                                            @endforeach
                                                        </select> -->
                                                        <select class="form-control select2" multiple name="candidate_group_id[]" id="candidate_group_id">
                                                            @foreach ($candidateGroups as $group)
                                                                <option value="{{ $group->id }}" {{ (collect(old('candidate_group_id'))->contains($group->id)) ? 'selected':'' }}>
                                                                    {{ $group->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    @elseif ($tab === 'pricing')
                                                        ...
                                                        <input type="text" name="{{ $field }}" id="{{ $field }}" class="form-control"
                                                            value="{{ old($field, 0.00) }}">

                                                    @elseif ($field === 'allow_retake')
                                                        <input type="checkbox" name="allow_retake" id="allow_retake" value="1" {{ old('allow_retake') ? 'checked' : '' }}>
                                                    ...
                                                    @elseif ($field === 'exam_type')
                                                        <select name="exam_type" id="exam_type" class="form-control">
                                                            ...
                                                            <option value="grading">A-F Grading</option>
                                                            <option value="good_excellent">Good/Excellent</option>
                                                        </select>

                                                        <!-- Grading Box (A-F Grading) -->
                                                        <div id="grading_box" class="grading_box" style="display: none;">
                                                        ...
                                                        <!-- Pass/Fail Box -->
                                                        <div id="pass_fail_box" class="grading_box" style="display: none;">
                                                            <label>Minimum Required Score</label>
                                                            <div class="input-group">
                                                                <input type="number" name="pass_fail_min_score" class="form-control"
                                                                    placeholder="Minimum Pass Score" value="50">
                                                            </div>
                                                        </div>

                                                        <!-- Good/Excellent Box -->
                                                        <div id="good_excellent_box" class="grading_box" style="display: none;">
                                                    ...
                                                    @else
                                                        <label for="{{ $field }}">{{ __($label) }}</label>
                                                        <input type="text" name="{{ $field }}" id="{{ $field }}" class="form-control"
                                                            placeholder="{{ __('Enter ' . $label) }}" value="{{ old($field) }}">
                                                    @endif
                                                </div>
                                            @endforeach
                                        </div>
