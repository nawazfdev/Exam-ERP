...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">{{ __('Create Industry') }}</h1>
            </div>
                ...
                <div class="card">
                    <form action="{{ route('Industries.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="card-body">
                                ...
                                <label for="industrycategory_id">{{ __('Category') }}</label>
                            <select class="form-control @error('industrycategory_id') is-invalid @enderror"
                                id="industrycategory_id" name="industrycategory_id" required>
                                <option value="">{{ __('Select a Category') }}</option>
                                @foreach($industrycategories as $industrycategory)
                                    <option value="{{ $industrycategory->id }}" {{ old('industrycategory_id') == $industrycategory->id ? 'selected' : '' }}>
                                        {{ $industrycategory->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('industrycategory_id')
                            ...
                            <div class="input-group mb-3">
                                <input type="text" name="title" id="title"
                                    ...
                                    placeholder="{{ __('Enter Industry Title') }}" value="{{ old('title') }}" required>
                                @error('title')
                                    <span class="error invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
...
<script>
    $(document).ready(function() {
        ...
        function uploadImage(file) {
            ...
            $.ajax({
                url: "{{ route('Industries.uploadImage') }}",
                method: "POST",
                data: data,
                processData: false,
                contentType: false,
